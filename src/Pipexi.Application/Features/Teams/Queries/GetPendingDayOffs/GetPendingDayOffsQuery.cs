...
namespace Pipexi.Application.Features.Teams.Queries.GetPendingDayOffs;

public sealed record GetPendingDayOffsQuery(Guid OrganizationId)
    : IQuery<Result<IReadOnlyCollection<PendingDayOffDto>>>;

public sealed class Handler : IRequestHandler<GetPendingDayOffsQuery, Result<IReadOnlyCollection<PendingDayOffDto>>>
{
    ...
    public async Task<Result<IReadOnlyCollection<PendingDayOffDto>>> Handle(
        GetPendingDayOffsQuery request,
        CancellationToken cancellationToken)
    {
        var organization = await _organizationRepository.GetByIdAsync(request.OrganizationId, cancellationToken);
        if (organization is null)
        {
            return Result<IReadOnlyCollection<PendingDayOffDto>>.Failure(
        ...
        if (teams.Count == 0)
        {
            return Result<IReadOnlyCollection<PendingDayOffDto>>.Success([]);
        }
        ...
        var pendingDayOffs = await _teamMemberDayOffRepository.ListPendingByTeamMemberIdsAsync(
            teamMemberIds, cancellationToken);

        if (pendingDayOffs.Count == 0)
        {
            return Result<IReadOnlyCollection<PendingDayOffDto>>.Success([]);
        }
        ...
        return Result<IReadOnlyCollection<PendingDayOffDto>>.Success(dtos);
    }
}
